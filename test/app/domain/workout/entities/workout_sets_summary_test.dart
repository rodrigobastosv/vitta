...
void main() {
  ...
  test('groups identical sets into the n×reps · load form', () {
    ...
    expect(WorkoutSetsSummary.format(sets: sets, unitSystem: .metric, l10n: l10n), '4×10 · 40 kg');
  });
  ...
  test('falls back to a set count and top load when the sets differ', () {
    ...
    expect(WorkoutSetsSummary.format(sets: sets, unitSystem: .metric, l10n: l10n), '2 sets · 45 kg');
  });

  test("converts the load into the reader's unit system", () {
    final sets = [WorkoutSetFactory.build(weightKg: 90)];

    expect(WorkoutSetsSummary.format(sets: sets, unitSystem: .imperial, l10n: l10n), '1×10 · 198.4 lb');
  });
  ...
  test('shows bodyweight rather than a zero load', () {
    ...
    expect(WorkoutSetsSummary.format(sets: sets, unitSystem: .metric, l10n: l10n), '2×12 · Bodyweight');
  });

  test('is null for an exercise with no previous sets', () {
    expect(WorkoutSetsSummary.format(sets: const [], unitSystem: .metric, l10n: l10n), isNull);
  });
  ...
  test('sums cardio efforts into total time and distance', () {
    ...
    expect(WorkoutSetsSummary.format(sets: sets, unitSystem: .metric, l10n: l10n), '30 min · 6 km');
  });
  ...
  test('omits distance from a cardio summary when none was logged', () {
    ...
    expect(WorkoutSetsSummary.format(sets: sets, unitSystem: .metric, l10n: l10n), '20 min');
  });
}
