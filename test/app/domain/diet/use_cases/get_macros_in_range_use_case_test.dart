...
void main() {
  test('returns the macros grouped by date for the given range', () async {
    ...
    final useCase = UseCasesFactories.buildGetMacrosInRangeUseCase(dietRepository: dietRepository);
    final from = DateTime(2026, 7);
    final to = DateTime(2026, 7, 31);
    final macrosByDate = {
      DateTime(2026, 7, 5): DailyMacros(entries: [FoodLogEntryFactory.build()]),
    };
    when(() => dietRepository.getMacrosInRange(from: from, to: to)).thenAnswer((_) async => Success(macrosByDate));
