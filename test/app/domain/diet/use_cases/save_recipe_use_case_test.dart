...
void main() {
  ...
  test('saves an ingredient that is not in the catalog yet before linking it', () async {
    ...
    when(
      () => dietRepository.saveFood(
        food: any(named: 'food', that: isA<Food>().having((food) => food.source, 'source', FoodSource.recipe)),
      ),
    ).thenAnswer((_) async => Success(FoodFactory.build(id: 'recipe-food-1')));
    when(
      () => dietRepository.createRecipe(
    ...
    await UseCasesFactories.buildSaveRecipeUseCase(dietRepository: dietRepository)(
      draft: RecipeDraft(
        name: 'Porridge',
        ingredients: [RecipeIngredient(food: unsavedFood, quantityGrams: 80)],
      ),
    );
  ...
  test('leaves an ingredient that already has an id alone', () async {
    final dietRepository = MockDietRepository();
    when(() => dietRepository.saveFood(food: any(named: 'food'))).thenAnswer((_) async => Success(FoodFactory.build(id: 'recipe-food-1')));
    when(
      () => dietRepository.createRecipe(
        foodId: any(named: 'foodId'),
        ingredients: any(named: 'ingredients'),
  ...
  test('saves the recipe itself as a food carrying the rolled-up macros', () async {
    ...
    await UseCasesFactories.buildSaveRecipeUseCase(dietRepository: dietRepository)(
      draft: RecipeDraft(
        name: 'Porridge',
        ingredients: [RecipeIngredientFactory.build(food: FoodFactory.build(caloriesPer100g: 100), quantityGrams: 200)],
      ),
    );
