...
No cubit models loading as a state variant anymore (no `XLoading`) — it's a presentation event instead, shown as one global overlay instead of each page switching on a loading state. `LoadingPresentationEvent` (`lib/app/presentation/general/loading_presentation_event.dart`) isn't a concrete event type to share — it's an interface (`bool get isLoading`) that each cubit's own presentation event type implements for its loading variants, e.g. `DietPresentationEvent.showLoading`/`.hideLoading`. A cubit emits it around an async load: `emitPresentation(DietPresentationEvent.showLoading)` before the await, `.hideLoading` after, then `emit(...)` the real state — see `DietCubit.loadToday()`/`WaterCubit.loadToday()`/`FoodSearchCubit.search()`. Because there's no loading state, a `PresentationCubit`'s **initial state is a "loaded but empty" value** (e.g. `DietCubit`'s initial state is `DietLoaded(dailyMacros: const DailyMacros(entries: []))`, not a null/loading placeholder) so the sealed state type only ever needs a loaded/error case — `WaterCubit` and `FoodSearchCubit`'s `FoodSearchIdle` follow the same idea. `VTPage`'s `BlocPresentationListener` checks `event is LoadingPresentationEvent` and turns `isLoading` into `context.showLoading()`/`context.hideLoading()` (`lib/app/core/loading/loading_extensions.dart`, wrapping `loader_overlay`'s `context.loaderOverlay`), and `main.dart` wraps `MaterialApp.router`'s `builder:` in a single `LoaderOverlay` for the whole app — pages/cubits never touch `loader_overlay` directly, only through those two extension methods. The overlay's visual is `VTLoadingOverlayIndicator` (design system, see below).
