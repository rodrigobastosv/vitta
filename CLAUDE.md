...
`flutter_bloc` Cubits. A Cubit that loads data extends `PresentationCubit<S>` (`lib/app/presentation/general/presentation_cubit.dart`) instead of `Cubit<S>` directly — it mixes in `bloc_presentation`'s `BlocPresentationMixin<S, LoadingPresentationEvent>`, giving it `emitPresentation(...)` for one-off side effects that aren't part of state. Pages that own such a cubit use `VTPage<C, S>` (`lib/app/presentation/general/vt_page.dart`), which now does three things at once: creates the cubit (`create:`, replacing a page-level `BlocProvider`), wires `BlocBuilder` (handing cubit + state to `builder:`, so pages never call `context.read` for their own cubit), and listens for `LoadingPresentationEvent`s to drive the global loading overlay (see Loading below). A cubit with no async load (`AppCubit`, `OnboardingCubit`'s create-account flow) doesn't need `VTPage` — `SettingsPage` just uses a plain `BlocBuilder<AppCubit, AppState>` since `AppCubit`'s state is available synchronously from construction.

## Loading

No cubit models loading as a state variant anymore (no `XLoading`) — it's a presentation event instead, shown as one global overlay instead of each page switching on a loading state. `LoadingPresentationEvent` (`show`/`hide`, `lib/app/presentation/general/loading_presentation_event.dart`) is the one shared event type every `PresentationCubit` emits around an async load: `emitPresentation(LoadingPresentationEvent.show)` before the await, `.hide` after, then `emit(...)` the real state — see `DietCubit.loadToday()`/`WaterCubit.loadToday()`/`FoodSearchCubit.search()`. Because there's no loading state, a `PresentationCubit`'s **initial state is a "loaded but empty" value** (e.g. `DietCubit`'s initial state is `DietLoaded(dailyMacros: const DailyMacros(entries: []))`, not a null/loading placeholder) so the sealed state type only ever needs a loaded/error case — `WaterCubit` and `FoodSearchCubit`'s `FoodSearchIdle` follow the same idea. `VTPage`'s `BlocPresentationListener` turns `show`/`hide` into `context.showLoading()`/`context.hideLoading()` (`lib/app/core/loading/loading_extensions.dart`, wrapping `loader_overlay`'s `context.loaderOverlay`), and `main.dart` wraps `MaterialApp.router`'s `builder:` in a single `LoaderOverlay` for the whole app — pages/cubits never touch `loader_overlay` directly, only through those two extension methods. The overlay's visual is `VTLoadingOverlayIndicator` (design system, see below).
...
- Cubits: `bloc_test`'s `blocTest`, building every mock and the cubit inside `build:` (not a separate `setUp:` — that would need `late` to share the mock between the two callbacks). If a mock must also be reached from `verify:`, declare it as a `final` local right above that one `blocTest(...)` call, not hoisted to the file/group level. A `PresentationCubit`'s state-transition tests stay in `blocTest` as usual (its `expect:` no longer includes a loading state, since there isn't one); its `LoadingPresentationEvent`s get their own `blocPresentationTest` (from `bloc_presentation_test`, same file, same `build:` shape) asserting `expectPresentation: () => [LoadingPresentationEvent.show, LoadingPresentationEvent.hide]` — see `diet_cubit_test.dart`/`water_cubit_test.dart`/`food_search_cubit_test.dart`.
